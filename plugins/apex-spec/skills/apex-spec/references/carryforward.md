...
Run after completing a phase, after `infra`, and before `documents`. If there
are no material discoveries, produce a concise no-op summary and still hand off
to `documents`. Do not jump directly to `plansession` from here: the next
command is `documents`, then `phasebuild` only if another phase remains, and
only then does the workflow return to `plansession` for the new phase.

## Rules

1. **Autonomous execution** - do not ask questions, request approval, or wait for human feedback
2. **CONSIDERATIONS.md: 600-line limit STRICT** - trim older/less relevant items if needed
3. **SECURITY-COMPLIANCE.md: 1000-line limit STRICT** - synthesize, don't append raw session reports
4. **Active Concerns**: max 20 items total (5 per subcategory)
5. **Lessons Learned**: max 30 items total
6. **Resolved**: max 15 items, rotate out after 2 phases
7. **Conciseness**: each item 1-3 lines max, include phase number `[PNN]`
8. **ASCII-only characters** and Unix LF line endings
...
- If phase not complete, make no changes and report `Next command: plansession`
...
Report what was updated or why no material update was needed.
...
- After `documents`: run `phasebuild` only if PRD.md still defines another unfinished phase
...
Summary:
- Carryforward updated institutional memory and security posture for Phase NN
- Key additions: [brief list or "none - no material additions"]

Next command: `documents`
Reason: carryforward is complete; documents is the next Phase Transition command before phasebuild can create another phase.
```

**Monorepo** (add package breakdown after Key additions and keep the same Summary and Next command block):
```
Package breakdown:
