...
The Apex Spec System has 24 commands total: 14 staged workflow commands and 10
utility commands. When porting from a Claude Code plugin to a Codex CLI Agent
Skill, we needed to decide how to structure the skill directory so that all 24
commands are accessible without polluting the skill namespace.

## Options Considered

1. **Single Compound Skill** -- One SKILL.md containing all 24 command
...
2. **Individual Skills per Command** -- 24 separate skill directories
   (apex-initspec/, apex-createprd/, etc.), each independently discoverable.
   Pro: granular invocation. Con: pollutes the skill namespace with 24 entries;
