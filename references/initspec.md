...
2. **Never overwrite existing `.spec_system/state.json`**; repair missing structure additively
3. **ASCII-only characters** (0-127) in all generated files
4. **Unix LF line endings** only
5. **Minimal structure** - don't over-engineer the initial setup
6. **Evidence-backed defaults** - infer project metadata from repository files, or use safe placeholders that `createprd` can replace
...
If already initialized, do not reinitialize and do not reset state. Repair only
missing directories or starter files that can be created without overwriting
existing content, then report the current state and the correct next command.

### 2. Gather Project Information

Derive project information without prompting:
- **Project name**: Use package metadata, pyproject metadata, README title, or
  the repository directory name, in that order
- **Project description**: Use package metadata, README opening summary, or
  "Project requirements to be generated by createprd"
- **First phase name**: Use "Foundation" unless an existing PRD or requirements
  source clearly names the first phase
...
2. If `detect_monorepo()` returns `detected: true`:
   - Record the detected indicator and discovered packages in the output summary
   - Set `monorepo: true` and store the packages array in state.json (Step 4)
   - If package detection is partial or contradictory, leave `monorepo: null`,
     record the evidence, and allow createprd to refine it later
...
**Important**: Never set `monorepo: false` during initspec. Only `null` (unknown) or `true` (detected). The createprd workflow step handles the `false` determination.
...
**If monorepo detected in Step 3a**, set `"monorepo": true` and add the packages array:
...
- Critique code, not people; raise concrete concerns rather than make demands
...
**If monorepo detected in Step 3a**, append a Workspace Structure section to CONVENTIONS.md:
...
### 10. Copy Scripts Locally

Copy scripts locally by default so initialized projects have deterministic,
...
Summary:
- Initialized .spec_system/ with starter PRD, state, conventions, audit, archive, and scripts
- Project name: [project name]
- Monorepo: [true with N packages | null]

Next command: `createprd`
Reason: the starter PRD is only a scaffold; createprd must convert available requirements and repository evidence into an actionable PRD before phasebuild creates sessions.
```
